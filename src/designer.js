...
layui.use(['layer', 'form', 'colorpicker'], function () {
  ...
  class UserTaskModel extends RectNodeModel {
    getNodeStyle() { var s = super.getNodeStyle(); s.radius = 8; return applyNodeStyle(this, s, '#e6f7ff', '#1890ff'); }
    getTextStyle() { return applyNodeTextStyle(this, super.getTextStyle()); }
  }
  ...
  document.getElementById('btn-snapshot').onclick = function () {
    ...
    html2canvas(document.querySelector('#graph'), { backgroundColor: '#fafafa' }).then(function (canvas) {
      ...
      link.download = 'flowchart.png'; link.href = canvas.toDataURL(); link.click();
  ...
  document.getElementById('btn-save').onclick = function () {
    ...
    layer.confirm('确定要保存当前流程图吗？', { icon: 3, title: '保存确认' }, function (index) {
      ...
      $.ajax({
        ...
        data: { act: 'Save_Navigator_DiagramData', data: jsonStr },
  ...
  // 返回导航页
  document.getElementById('btn-back-nav').onclick = function () {
